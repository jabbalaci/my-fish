...
function xrates --argument value -d "Currency rates of HUF, EUR and USD"
	...
	set -l api_call "https://api.frankfurter.dev/v2/rates?base=HUF&quotes=EUR,USD"
	set -l json (curl -s $api_call)
	# echo $json
	set -l eur (echo $json | jq '.[] | select(.quote=="EUR") | .rate')
	set -l usd (echo $json | jq '.[] | select(.quote=="USD") | .rate')
